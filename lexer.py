...
class Lexer:
    ...
    def op_type(self) -> Tt:
        for op in reversed(self.ops):
            ...
            if tt is not None:
                return tt
        raise UnknownTokenError("operator", self.curr())
    ...
    def move_on(self, expect: str | list[str]) -> bool:
        c = self.peek()
        if c in expect:
            self.next()
            return True
    ...
    def op(self) -> Token:
        tt = self.op_type()
        self.next()  # Consume the op
        return self.consume(tt)
